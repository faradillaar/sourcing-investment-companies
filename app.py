...
def ensure_company_type_table() -> None:
    ...
    """)
    db_execute("""
        create index if not exists idx_psc_company_number on public.psc(company_number)
    """)
    ...
    db_execute("""
        create table if not exists public.charges (
            ...
            delivered_on        date,
            satisfied_on        date,
            secured_details     text,
            raw_json            jsonb,
            last_fetched_at     timestamptz
        )
    """)
    db_execute("""
        create index if not exists idx_charges_company_number on public.charges(company_number)
    """)
    ...
    """)
    db_execute("""
        create index if not exists idx_filing_history_company_number on public.filing_history(company_number)
    """)
...
def fetch_psc_api(company_number: str) -> List[Dict[str, Any]]:
    try:
        payload = api_get(f"/company/{company_number}/persons-with-significant-control")
        return payload.get("items", [])
    except Exception:
        return []


def fetch_charges_api(company_number: str) -> List[Dict[str, Any]]:
    try:
        payload = api_get(f"/company/{company_number}/charges")
        return payload.get("items", [])
    except Exception:
        return []


def fetch_filing_history_api(company_number: str, items_per_page: int = 25) -> List[Dict[str, Any]]:
    try:
        payload = api_get(f"/company/{company_number}/filing-history", params={"items_per_page": items_per_page})
        return payload.get("items", [])
    except Exception:
        return []
...
# =========================================================
def get_cached_psc(company_number: str) -> Tuple[List[tuple], Optional[datetime]]:
    rows = db_fetchall("""
        select
            name,
            kind,
        ...
        from public.psc
        where company_number = %s
        order by notified_on desc nulls last, name asc
    """, (company_number,))
...
def replace_psc(company_number: str, items: List[Dict[str, Any]]) -> None:
    db_execute("delete from public.psc where company_number = %s", (company_number,))

    for item in items:
        natures = item.get("natures_of_control") or []
        db_execute("""
            insert into public.psc (
                company_number, name, kind, nationality, country_of_residence,
                natures_of_control, notified_on, ceased_on, raw_json, last_fetched_at
            )
        ...
        """, (
            ...
            item.get("country_of_residence"),
            to_json(natures),
            parse_date(item.get("notified_on")),
            parse_date(item.get("ceased_on")),
            to_json(item),
        ))


def get_or_refresh_psc(company_number: str) -> pd.DataFrame:
    cached_rows, last_fetched_at = get_cached_psc(company_number)
    ...
    items = fetch_psc_api(company_number)
    replace_psc(company_number, items)

    cached_rows, _ = get_cached_psc(company_number)
    if not cached_rows:
...
# CHARGES
# =========================================================
def get_cached_charges(company_number: str) -> Tuple[List[tuple], Optional[datetime]]:
    rows = db_fetchall("""
        select
            charge_code,
            status,
...
def replace_charges(company_number: str, items: List[Dict[str, Any]]) -> None:
    db_execute("delete from public.charges where company_number = %s", (company_number,))

    for item in items:
        classification = (item.get("classification") or {}).get("description")
        secured_details = (item.get("particulars") or {}).get("description")
        db_execute("""
            insert into public.charges (
                company_number, charge_code, status, classification,
                created_on, delivered_on, satisfied_on, secured_details,
                raw_json, last_fetched_at
            )
            values (%s, %s, %s, %s, %s, %s, %s, %s, %s::jsonb, now())
        """, (
            company_number,
...
def get_or_refresh_charges(company_number: str) -> pd.DataFrame:
    cached_rows, last_fetched_at = get_cached_charges(company_number)
    ...
    items = fetch_charges_api(company_number)
    replace_charges(company_number, items)

    cached_rows, _ = get_cached_charges(company_number)
    if not cached_rows:
        return pd.DataFrame(columns=[
            "charge_code", "status", "classification", "created_on",
            "delivered_on", "satisfied_on", "secured_details", "last_fetched_at"
...
# FILING HISTORY
# =========================================================
def get_cached_filing_history(company_number: str) -> Tuple[List[tuple], Optional[datetime]]:
    rows = db_fetchall("""
        select
            transaction_id,
            filing_type,
    ...
    last_fetched_at = rows[0][6]
    return rows, last_fetched_at


def replace_filing_history(company_number: str, items: List[Dict[str, Any]]) -> None:
    db_execute("delete from public.filing_history where company_number = %s", (company_number,))

    for item in items:
        db_execute("""
            insert into public.filing_history (
                company_number, transaction_id, filing_type, category,
                description, date, pages, raw_json, last_fetched_at
            )
...
def get_or_refresh_filing_history(company_number: str) -> pd.DataFrame:
    cached_rows, last_fetched_at = get_cached_filing_history(company_number)
    ...
    items = fetch_filing_history_api(company_number)
    replace_filing_history(company_number, items)

    cached_rows, _ = get_cached_filing_history(company_number)
    if not cached_rows:
        return pd.DataFrame(columns=[
            "transaction_id", "filing_type", "category",
            "description", "date", "pages", "last_fetched_at"
...
def process_single_keyword(keyword: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if company_number:
            ...
            get_or_refresh_psc(company_number)
            get_or_refresh_charges(company_number)
            get_or_refresh_filing_history(company_number)
...
# SCORING
# =========================================================
def calculate_investment_score(profile: Dict[str, Any], officers_df: pd.DataFrame, psc_df: pd.DataFrame, charges_df: pd.DataFrame, filing_df: pd.DataFrame = None):
    score = 0
    reasons = []
    ...
    # PSC scoring
    if not psc_df.empty:
        active_psc = psc_df[psc_df["ceased_on"].isna()] if "ceased_on" in psc_df.columns else psc_df
        if len(active_psc) > 0:
            score += 10
            reasons.append(f"Has {len(active_psc)} active PSC registered (+10)")
        else:
            score -= 10
            reasons.append("No active PSC (Persons with Significant Control) registered (-10)")
    else:
        score -= 10
        reasons.append("No PSC data — possible shell company or exempt entity (-10)")

    # Charges scoring
    if not charges_df.empty:
        outstanding = charges_df[charges_df["status"] == "outstanding"] if "status" in charges_df.columns else pd.DataFrame()
        satisfied = charges_df[charges_df["status"] == "satisfied"] if "status" in charges_df.columns else pd.DataFrame()

        if len(outstanding) == 0 and len(satisfied) > 0:
            score += 10
            reasons.append(f"All {len(satisfied)} charges satisfied — clean debt record (+10)")
        elif len(outstanding) > 0 and len(satisfied) > 0:
            score += 5
            reasons.append(f"{len(outstanding)} outstanding + {len(satisfied)} satisfied charges — has active debt (+5)")
        elif len(outstanding) > 0:
            score -= 5
            reasons.append(f"{len(outstanding)} outstanding charges with no satisfied history (-5)")
    else:
        reasons.append("No charges on record (+0)")

    # Filing history scoring
    if filing_df is not None and not filing_df.empty:
        if "date" in filing_df.columns:
            try:
                latest = pd.to_datetime(filing_df["date"], errors="coerce").max()
                if pd.notna(latest):
                    months_since = (datetime.utcnow().date() - latest.date()).days / 30
                    if months_since <= 12:
                        score += 10
                        reasons.append(f"Filed within the last 12 months (+10)")
                    elif months_since <= 24:
                        score += 5
                        reasons.append(f"Filed within the last 24 months (+5)")
                    else:
                        score -= 10
                        reasons.append(f"No filing in over 2 years — potential red flag (-10)")
            except Exception:
                pass
        total_filings = len(filing_df)
        reasons.append(f"{total_filings} filings on record (info)")
    else:
        reasons.append("No filing history found (+0)")
...
def render_main_page():
    ...
    with st.spinner("Loading PSC & charges..."):
        psc_df = get_or_refresh_psc(selected_company_number)
        charges_df = get_or_refresh_charges(selected_company_number)
    ...
    with tab3:
        st.markdown("### Persons with Significant Control (PSC)")
        if psc_df.empty:
            st.info("No PSC data found for this company.")
        else:
            active_psc = psc_df[psc_df["ceased_on"].isna()].shape[0] if "ceased_on" in psc_df.columns else len(psc_df)
            p1, p2 = st.columns(2)
            p1.metric("Total PSC", len(psc_df))
            p2.metric("Active PSC", int(active_psc))
            show_psc = psc_df.drop(columns=["last_fetched_at"], errors="ignore").copy()
            show_psc["natures_of_control"] = show_psc["natures_of_control"].apply(
                lambda x: ", ".join(x) if isinstance(x, list) else str(x or "")
            )
            show_psc = show_psc.rename(columns={
                "name": "Name",
                "kind": "Kind",
                "nationality": "Nationality",
                "country_of_residence": "Country of Residence",
                "natures_of_control": "Natures of Control",
                "notified_on": "Notified On",
                "ceased_on": "Ceased On",
            })
            st.dataframe(show_psc, use_container_width=True, hide_index=True)

    with tab4:
        st.markdown("### Charges")
        if charges_df.empty:
            st.info("No charges found for this company.")
        else:
            outstanding = charges_df[charges_df["status"] == "outstanding"].shape[0] if "status" in charges_df.columns else 0
            satisfied = charges_df[charges_df["status"] == "satisfied"].shape[0] if "status" in charges_df.columns else 0
            c1, c2, c3 = st.columns(3)
            c1.metric("Total Charges", len(charges_df))
            c2.metric("Outstanding", int(outstanding))
            c3.metric("Satisfied", int(satisfied))
            show_charges = charges_df.drop(columns=["last_fetched_at"], errors="ignore").copy()
            show_charges = show_charges.rename(columns={
                "charge_code": "Charge Code",
                "status": "Status",
                "classification": "Classification",
                "created_on": "Created On",
                "delivered_on": "Delivered On",
                "satisfied_on": "Satisfied On",
                "secured_details": "Secured Details",
            })
            st.dataframe(show_charges, use_container_width=True, hide_index=True)

    with tab5:
        st.markdown("### Filing History (latest 25)")
        if filing_df.empty:
            st.info("No filing history found for this company.")
        else:
            st.metric("Total Filings Shown", len(filing_df))
            show_filing = filing_df.drop(columns=["last_fetched_at", "transaction_id"], errors="ignore").copy()
            show_filing = show_filing.rename(columns={
                "filing_type": "Type",
                "category": "Category",
...
def _render_browse_section():
    ...
    # ── Company detail viewer (reads from DB cache, no API call if already stored)
    st.markdown("#### View Company Detail")
    options = [f"{row['title']} ({row['company_number']}) • {row['company_status']}" for _, row in df.iterrows()]
    selected = st.selectbox("Select a company to view details", ["— select —"] + options, key="b_detail_select")

    if selected and selected != "— select —":
        match = re.search(r"\(([A-Za-z0-9]+)\)", selected)
        selected_cn = match.group(1) if match else None

        if selected_cn:
            with st.spinner("Loading details..."):
                profile = get_or_refresh_company_profile(selected_cn)
                officers_df = get_or_refresh_officers(selected_cn)
                psc_df = get_or_refresh_psc(selected_cn)
                charges_df = get_or_refresh_charges(selected_cn)
            ...
            with tab2:
                ...
                if officers_df.empty:
                    ...
                    show_officers = officers_df.drop(columns=["last_fetched_at"], errors="ignore").copy()
                    show_officers = show_officers.rename(columns={
                        "officer_name": "Officer Name", "officer_role": "Role",
                        "appointed_on": "Appointed On", "resigned_on": "Resigned On",
                        "nationality": "Nationality", "occupation": "Occupation",
                        "country_of_residence": "Country of Residence",
                    })
                    st.dataframe(show_officers, use_container_width=True, hide_index=True)
                    active_officers = int(officers_df["resigned_on"].isna().sum()) if "resigned_on" in officers_df.columns else 0
                    m1, m2 = st.columns(2)
                    m1.metric("Total Officers", len(officers_df))
                    m2.metric("Active Officers", active_officers)

            with tab3:
                st.markdown("### Persons with Significant Control (PSC)")
                if psc_df.empty:
                    st.info("No PSC data found.")
                else:
                    active_psc = int(psc_df["ceased_on"].isna().sum()) if "ceased_on" in psc_df.columns else len(psc_df)
                    p1, p2 = st.columns(2)
                    p1.metric("Total PSC", len(psc_df))
                    p2.metric("Active PSC", active_psc)
                    show_psc = psc_df.drop(columns=["last_fetched_at"], errors="ignore").copy()
                    show_psc["natures_of_control"] = show_psc["natures_of_control"].apply(
                        lambda x: ", ".join(x) if isinstance(x, list) else str(x or "")
                    )
                    show_psc = show_psc.rename(columns={
                        "name": "Name", "kind": "Kind", "nationality": "Nationality",
                        "country_of_residence": "Country of Residence",
                        "natures_of_control": "Natures of Control",
                        "notified_on": "Notified On", "ceased_on": "Ceased On",
                    })
                    st.dataframe(show_psc, use_container_width=True, hide_index=True)
            ...
            with tab4:
                ...
                if charges_df.empty:
                    ...
                    outstanding = int((charges_df["status"] == "outstanding").sum()) if "status" in charges_df.columns else 0
                    satisfied = int((charges_df["status"] == "satisfied").sum()) if "status" in charges_df.columns else 0
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Total Charges", len(charges_df))
                    c2.metric("Outstanding", outstanding)
                    c3.metric("Satisfied", satisfied)
                    show_charges = charges_df.drop(columns=["last_fetched_at"], errors="ignore").copy()
                    show_charges = show_charges.rename(columns={
                        "charge_code": "Charge Code", "status": "Status",
                        "classification": "Classification", "created_on": "Created On",
                        "delivered_on": "Delivered On", "satisfied_on": "Satisfied On",
                        "secured_details": "Secured Details",
                    })
                    st.dataframe(show_charges, use_container_width=True, hide_index=True)
